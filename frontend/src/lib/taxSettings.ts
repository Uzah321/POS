// Mirrors the backend tax logic in SaleController::store — a product marked
// non-taxable never carries tax regardless of store settings; otherwise a
// product's own tax rate (if assigned) overrides the store-wide rate, when
// tax is enabled. Used to show a live tax preview in the cart that matches
...
export function effectiveTaxRate(product: any, storeSettings: Record<string, any> | undefined): number {
  ...
  if (product?.is_taxable === false) return 0;
