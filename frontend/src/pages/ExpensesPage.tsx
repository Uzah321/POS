...
import { expensesApi, branchesApi } from '../api';
                  ...
                  : expenses.map((e: any) => (
                    <tr key={e.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        <RowActionsMenu
                          actions={[
                            { label: 'Edit', icon: <Edit size={14} />, onClick: () => setModal({ open: true, expense: e }) },
                            { label: 'Delete', icon: <Trash2 size={14} />, danger: true, onClick: () => { if (confirm('Delete this expense?')) deleteMutation.mutate(e.id); } },
                          ]}
                        />
                      </td>
                    </tr>
